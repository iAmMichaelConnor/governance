...
| **Governance proposal**                  | Governance proposal **#4** on `Governance` `0x1102471eb3378fee427121c9efcea452e4b6b75e`, whose payload is the `GSEPayload` wrapper `0x1744be5cf3f314bfb2999db8acec1748916e74b5` created by `GovernanceProposer.submitRoundWinner` around the address above |
...
| `GSEPayload` (wrapper)                   | `0x1744be5cf3f314bfb2999db8acec1748916e74b5` | Created by `GovernanceProposer`; what `Governance` stores for proposal #4                               |
...
| Default public-setup allowlist    | `AuthRegistry._set_authorized`, `AuthRegistry.set_authorized` and `FeeJuice._increase_public_balance` (node default, `allowed_public_setup.ts`). The `AuthRegistry` instance referenced by nodes from release `5.0.1` onward is `0x1e8e7e73c592a1b1c9199b4b655ddc7a16fa8a8488df595610b71d3dc1cc666c`; see open question 1. |
...
The same payload was executed on Sepolia as Governance proposal **#5** on `0xCAf7447721447B22Cd0076aC7C63877c3AFD329F` (executed 2026-07-13 12:53:00 UTC in tx `0xa5fea6306e52e2b69d7b413b89aba0bf83f900cac1cd50d30da79a7f704ab043`), after two earlier v5 release-candidate rollups had been registered by proposals #3 and #4. Addresses were read from the chain; the Sepolia payloads are not verified on Sourcify.
...
2. This AZUP was filed after execution. The process requires the payload to be merged and tagged here *before* deployment; no tag was created for AZUP-2 (this repository has no tags). Proposals #0–#2 on mainnet Governance also predate the AZUP process and have no AZUP.
